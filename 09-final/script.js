...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // starts the trail of tiny bubbles that blow off the cursor as it moves
  initCursorBubbles();
});
...
function initCursorBubbles() {
  ...
  let lastSpawn = 0;
  const minSpawnGap = 55; // ms between spawns, keeps the trail light on perf

  function handlePointerMove(x, y) {
    const now = performance.now();
    if (now - lastSpawn < minSpawnGap) return;
    lastSpawn = now;
    spawnCursorBubble(layer, x, y);
  }
  ...
  // lets the trail follow a finger on touch devices too
...
// creates one bubble at (x, y) with a random size, drift, and lifespan,
// then removes itself from the DOM once its rise/fade animation finishes
function spawnCursorBubble(layer, x, y) {
  const bubble = document.createElement('div');
  bubble.className = 'cursor-bubble';

  const size = 6 + Math.random() * 22;       // 6px – 28px
  const driftX = (Math.random() - 0.5) * 70; // -35px – 35px sideways drift
  const duration = 1.3 + Math.random() * 1.5; // 1.3s – 2.8s lifespan
